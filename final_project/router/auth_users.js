...
regd_users.put("/auth/review/:isbn", (req, res) => {
  ...
  var reV = req.query.reviews; // (localhost:5000/customer/auth/review/1?reviews=This book is very good) Hace referencia a la query string o parámetros dinámicos que pasemos en la url.
  
    const user = req.session.authorization.username;  //Retrieve username. re.session stores session data that matching with cookies in broser.
 ...
 regd_users.delete("/auth/review/:isbn", (req, res) => {
  var iisbn = req.params.isbn; // Carga el nombre del title del parámetro "isbn" enviada con la solicitud GET.
  var reV = req.query.reviews; // (Test: localhost:5000/customer/auth/review/1?reviews=This book is very good) Hace referencia a la query string o parámetros dinámicos que pasemos en la url.
  ...
  if (!isbnValid(iisbn)) {  
      ...
      for (var i = 0; i < keys.length; i++) { //Se crea una ciclo para iterar el objeto. El mensaje con status 400 No found, debe quedar fuera de esta iteración.
          ...
          delete (entries[i][1]["reviews"])[user]; //Borra el review from the username loaded in global.
